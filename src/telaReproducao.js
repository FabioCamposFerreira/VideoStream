...
class TelaReproducao extends React.Component {
    constructor(props) {
        super(props);
        this.state = { url: '/reproducao', autenticacao: localStorage.autenticacao, videoTitle: '', videoList: [], nextVideo: [], urlVideo: [] };
    }
        ...
        firebase.database().ref('videos/' + localStorage.telaReproducaoVideo.replace(/\"/g, "")).on('value',
            (snapshot) => {
                ...
                if (snapshot.val() != null) {
                    ...
                    localStorage.tituloVideo = 0;
                    localStorage.canal = 0;
                    localStorage.proximoVideo = 0;
                    localStorage.videoAnterior = 0;
                    localStorage.administradorCanal = 0;
                }
            ...
            firebase.database().ref('videos/' + localStorage.telaReproducaoVideo.replace(/\"/g, "")).on('value',
                (snapshot) => {
                    ...
                    if (snapshot.val() != null) {
                        ...
                        localStorage.setItem('imagemVideo', allData.localizacaoImagem);
                        localStorage.setItem("nextVideoChave", allData.proximoVideo);
                        //adiciona proximo video
                        var adicionarProximoVideo = this.adicionarProximoVideo;
                        firebase.database().ref('videos/' + localStorage.nextVideoChave).on('value',
                            (snapshot) => {
                                if (snapshot.val() != null) {
                                    adicionarProximoVideo(snapshot.val(), localStorage.nextVideoChave);
                                }
                            });
                    }
                    else {
    ...
    redirectToNextVideo = (chave) => {
        localStorage.telaReproducaoVideo = chave;
        firebase.database().ref('videos/' + chave.replace(/\"/g, "")).on('value',
            (snapshot) => {
                //verifica se existe video
                console.log(snapshot.val());
                if (snapshot.val() != null) {
        ...
        return (
            ...
            <div className={classes.body} >
                ...
                <p style={{ backgroundColor: "black", padding: 10 }}>
                    <video style={{ display: "block", marginLeft: "auto", marginRight: "auto", maxWidth: 853 }} controls="controls"
                        width="100%">
